...
class CreateTaskViewTest(StaticLiveServerTestCase):
    ...
    def test_remove_task(self):
        ...
        self.assertEqual(self.live_server_url + "/",self.driver.current_url, "Link redirects to other routes.")

        time.sleep(2)
        # Get rendered values
        linkButton = self.driver.find_element_by_css_selector("td:nth-of-type(3) > a")
        linkButton.click()
        time.sleep(2)

        # Verify that the task was removed
        self.assertEqual(self.live_server_url + "/",self.driver.current_url,"Link redirects to other routes.")
        message = self.driver.find_element_by_css_selector("tr").get_attribute("innerText").strip()
